...
internal interface IAudioClient
{
    ...
    ///     Flags to control creation of the stream. The client should set this parameter to 0 or to the
    ///     bitwise OR of one or more of the AUDCLNT_STREAMFLAGS_XXX Constants or the AUDCLNT_SESSIONFLAGS_XXX Constants.
    /// </param>
    /// <param name="hnsBufferDuration">
    ///     The buffer capacity as a time value. This parameter is of type REFERENCE_TIME and is
    ///     expressed in 100-nanosecond units. This parameter contains the buffer size that the caller requests for the buffer
    ///     that the audio application will share with the audio engine (in shared mode) or with the endpoint device (in
    ///     exclusive mode). If the call succeeds, the method allocates a buffer that is a least this large.
    /// </param>
    ...
    ///     period, call the IAudioClient::GetDevicePeriod method. If the AUDCLNT_STREAMFLAGS_EVENTCALLBACK stream flag is set
    ///     and AUDCLNT_SHAREMODE_EXCLUSIVE is set as the ShareMode, then hnsPeriodicity must be nonzero and equal to
    ///     hnsBufferDuration.
    /// </param>
    /// <param name="pFormat">
    ///     Pointer to a format descriptor. This parameter must point to a valid format descriptor of type
    ///     WAVEFORMATEX (or WAVEFORMATEXTENSIBLE).
    /// </param>
    /// <param name="audioSessionGuid">
    ///     Pointer to a session GUID. This parameter points to a GUID value that identifies the
    ///     audio session that the stream belongs to. If the GUID identifies a session that has been previously opened, the
    ///     method adds the stream to that session. If the GUID does not identify an existing session, the method opens a new
    ///     session and adds the stream to that session. The stream remains a member of the same session for its lifetime.
